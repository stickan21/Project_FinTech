...
def form():
    ...
    if form.is_submitted():
        ...
        res = fin.run(Datas)
        if res:
            result = {'status': 'success', 'data': round(float(res[0]), 3)}
        else:
            result = {'status': 'failed', 'data':''}

        return jsonify(result)
    return render_template('fintech/result.html', form=form)




# endpoint page
@fintech_caller.route('/result/')
def submit():
    return render_template('fintech/result.html')
